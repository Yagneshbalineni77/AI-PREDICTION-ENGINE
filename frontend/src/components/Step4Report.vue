        ...
        <div v-if="reportOutline" class="report-content-wrapper">
          ...
          <div class="report-header-block">
            <div class="report-meta">
              ...
              <button @click="downloadPDF" class="download-pdf-btn" v-if="reportOutline">
          ...
          <div class="sections-list">
            ...
            >
              ...
              <div class="section-body" v-show="isGeneratingPdf || !collapsedSections.has(idx)">
                <!-- Completed Content -->
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (Content生成Complete, but entire section may not haveComplete) -->
...
const downloadPDF = async () => {
  ...
  // Wait for Vue to update the DOM (uncollapse all sections)
  await nextTick()
  
  const element = leftPanel.value.querySelector('.report-content-wrapper')
  if (!element) {
    isGeneratingPdf.value = false
    return
  }
  
  // Temporarily hide the button itself during generation
  const btn = element.querySelector('.download-pdf-btn')
  if (btn) btn.style.display = 'none'

  const opt = {
    margin:       10,
    filename:     `Prediction_Report_${props.reportId || 'Export'}.pdf`,
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // Bulb icon - represents insight
  },
  'panorama_search': {
...
const parseInsightForge = (text) => {
  ...
  try {
    // 提取Analysis Questions
    const queryMatch = text.match(/Analysis Questions:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // 提取Prediction Scenarios
    const reqMatch = text.match(/Prediction Scenarios:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // Extract statistics - 匹配"Related Prediction Facts: X条"Format
    ...
    // 提取Sub-questions - Full extraction, no quantity limit
    ...
    // 提取Key Facts - Full extraction, no quantity limit
    ...
    // 提取Core Entities - 完整提取，ContainsSummary和Related Facts数
    ...
    // 提取Relationship Chains - Full extraction, no quantity limit
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // 提取History/Expired Facts - Full extraction, no quantity limit
    ...
    // 提取Related Entities - Full extraction, no quantity limit
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // 提取Number of Interviewees（如 "5 / 9 simulationsAgent"）
    ...
    // 提取Interviewee Selection Rationale
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // For example: 1. **Alumni_345 (index=1）**: as Wuhan University alumni...
        ...
        // Format2: - SelectionName (index X): Rationale
        // For example: - SelectionParent_601 (index 0): As parent group representative...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*Selection([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        // For example: - **Parent_601 (index 0）**: as parent group representative...
        if (!headerMatch) {
        ...
        if (name) {
          // Save previous person reasoning
          ...
          // Continuation of reasoning (excluding summary paragraph)
          currentReason.push(line.trim())
        }
      }
      
      // Save last person reasoning
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract title (e.g. "Student"、"Education Practitioners" etc.)
      ...
      // Extract answers - separate Twitter andReddit
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (backward compatible with oldFormat: case with only one platform marker)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer, copy as default display only when not placeholder
          ...
          // No platform markers (very oldFormat), treat whole as answer
          interview.twitterAnswer = answerText
        }
      }
      
      // 提取Key Quotes（兼容多种引号Format）
      const quotesMatch = block.match(/\*\*Key Quotes:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // Priority match > "text" Format
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // Fallback: match > "text" 或 > \u201Ctext\u201D(Chinese quotation marks)
        if (!quoteMatches) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/Found\s*(\d+)\s*条/)
    if (countMatch) result.count = parseInt(countMatch[1])
    
    // 提取Related Facts - Full extraction, no quantity limit
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Latest key facts related to temporal memory'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 支持两种编号Format：
      // 1. "Question X:" 或 "QuestionX:" （中文Format，后端新Format）
      // 2. "1. " 或 "\n1. " （Number+点，旧Format兼容）
      let matches = []
      let match
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // 分割失败：第一个QuestionReturn full answer, rest return empty
      return qIdx === 0 ? answer : ''
    }
    
    // 检查某个QuestionWhether has dual-platform answers (filter placeholders)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real answers (not placeholder text) andContent不同
...
const activeStep = computed(() => {
  ...
  // If no active, return the last one done 的步骤
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 title (## xxx), because section title already shown in outer layer
  ...
  // CleanupListAll whitespace between items
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // CleanupList开始Label后的空白
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // CleanupList结束Label前的空白
  ...
  // Handle line breaks - empty lines become paragraph breaks, single newlines become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Cleanup块级元素前后的 <br> Label
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Cleanup <p><br> Case of following block element (caused by extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Cleanup连续的 <br> Label
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // CleanupParagraph start immediately following block elementLabel前的 <br>
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // 修复非连续有序List的编号：当单项 <ol> 被段落ContentWhen separated, keep numbering increasing
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // 如果TaskCompleted, scroll to top; otherwise scroll to bottom to follow latest logs
            if (isComplete.value) {
...
// 提取Final AnswerContent - 从 LLM response 中提取章节Content
const extractFinalContent = (response) => {
  if (!response) return null
  
  // Try to extract <final_answer> Label内的Content
  ...
  // Try to find Final Answer: 后面的Content（支持多种Format）
...
/* Console Logs - 与 Step3Simulation.vue keep consistent */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
...
<style>
/* Global Print Styles to orchestrate clean PDF exports from the browser */
@media print {
  /* Hide UI elements not meant for PDF */
  .app-header, .panel-wrapper.left, .right-panel, .console-logs, 
  .download-pdf-btn, .next-step-btn, .collapse-icon {
    display: none !important;
  }
  
  /* Reset layout constraints to allow natural flow across pages */
  .main-view, .report-panel, .main-split-layout, .content-area, 
  .panel-wrapper, body, html {
  ...
  /* Force expand all sections regardless of current Vue state */
  .section-body {
    display: block !important;
  }
  
  /* Prevent awkward page breaks inside sections */
  .report-section-item, .report-header-block {
  ...
  .generated-content h2, .generated-content h3 {
    page-break-after: avoid;
    break-after: avoid;
  }
  
  /* Optimize typography colors for white paper */
  body {
