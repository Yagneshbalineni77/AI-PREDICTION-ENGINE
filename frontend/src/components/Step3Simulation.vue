          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- General fallback: unknown type or has content 但未被上述处理 -->
...
// FormatFormat simulation elapsed time (calculated from rounds and minutes per round)
...
// TwitterPlatform simulation elapsed time
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// RedditPlatform simulation elapsed time
...
const doStartSimulation = async () => {
  ...
  // 先重置所有Status, ensure not affected by previous simulation
  resetAllState()
...
// Track each platform last round for change detection and logging
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 检测模拟是否Completed（通过 runner_status 或平台CompleteStatus判断）
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Extra check: if backend has not yet updated runner_status，但平台已经报告Complete
...
// Check if all enabled platformsCompleted
const checkPlatformsCompleted = (data) => {
  // If no platform data, return false
  if (!data) return false
  
  // 检查各平台的CompleteStatus
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true
  
  // If at least one platformCompletecompleted, check if all enabled platforms haveComplete了
  // Determine if platform is enabled via actions_count (if count > 0 或 running 曾为 true）
  ...
  // Check if all enabled platforms haveCompleted
