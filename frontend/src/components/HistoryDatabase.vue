<template>
  ...
  >
    <!-- Background decoration: tech grid lines (only when there areProject时显示） -->
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        <!-- Card header：simulation_id 和 功能可用Status -->
        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-list" v-if="project.files && project.files.length > 0">
            ...
            <!-- If more files exist, show prompt -->
        ...
        <!-- Card Title（使用Simulation Requirement的前20字作为标题） -->
        <h3 class="card-title">{{ getSimulationTitle(project.simulation_requirement) }}</h3>

        <!-- Card Description（Simulation Requirement完整展示） -->
        ...
        <!-- Bottom Decoration Line (hover时展开) -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 'Simulate' and Step5 'Interaction' require a running simulation instance and cannot be played back.</span>
            </div>
          </div>
        </div>
      </Transition>
...
let pendingState = null  // Record pending targetStatus

// 卡片布局Configuration - adjust to wider ratio
...
const containerStyle = computed(() => {
  ...
  // Calculate actual required height:Row count * Card height + (Row count-1) * Spacing + 少量底部Spacing
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate current row card count, ensure row centering
    ...
    // Expand downward, increase spacing with titleSpacing
    ...
    const x = offset * 35
    // Adjust start position, close to title but maintaining properSpacing
...
// Formatformat date (show date part only)
...
// Format化 simulation_id 显示（截取前6digits)
...
// Formatround display (current round/total rounds)
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update pending targetStatus(record latest target regardless of animation stateStatus）
        ...
        // If animating, only recordStatus, process after animation ends
        if (isAnimating) return
        
        // 如果目标Status与当前Statussame, no processing needed
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // 使用防抖延迟Statustoggle, prevent rapid flickering
        // Shorter delay when expanding(50ms), longer delay when collapsing(200ms)to increase stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // 检查待执行StatusWhether still needs execution (may have been overridden by subsequent scroll)
          ...
          // 动画Completeunlock and check for pendingStatus变化
          setTimeout(() => {
            isAnimating = false
            
            // After animation ends, check for new pending executionStatus
...
/* 使用CSSBackground pattern creates fixedSpacing的正方形网格 */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
...
/* Low saturation color scheme - Morandi色系 */
...
/* ===== History Replay Detail Dialog样式 ===== */
.modal-overlay {
  position: fixed;
  top: 0;
  left: 0;
