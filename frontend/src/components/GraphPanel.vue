    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'GraphRAG Long/Short Term Memory Updating...' : 'Updating...' }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">A few items are still being processed. It is recommended to manually refresh the graph later.</span>
...
const showEdgeLabels = ref(true) // 默认Show Edge Labels
const expandedSelfLoops = ref(new Set()) // Expanded self-loop item
const showSimulationFinishedHint = ref(false) // Post-simulation notice
const wasSimulating = ref(false) // Track whether previously simulating

// Close simulation end notice
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // transition from simulating to not simulatingStatus，显示结束提示
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// Toggle self-loop item expand/折叠Status
...
const renderGraph = () => {
  ...
  // Process edge data, calculate same pair ofNodeedge count and index between
  const edgePairCount = {}
  const selfLoopEdges = {} // 按Node分组的自环边
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // 统计每对Nodeedge count between nodes, collect self-loops
  tempEdges.forEach(e => {
  ...
  // Record current processing for each pairNode的第几条边
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // 已处理的自环Node
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // 自环边 - 每个NodeOnly add one merged self-loop
    ...
    // Check if edge direction matches normalized direction (sourceUUID < 目标UUID）
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Calculate curvature: spread for multiple edges, single edge isStraight line
    let curvature = 0
    if (totalCount > 1) {
      // Evenly distribute curvature, ensure clear distinction
      // Curvature range increases with edge count, more edges means wider curvature range
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // If edge direction is opposite to normalized direction, flip curvature
      // Ensures all edges are distributed in the same reference frame without overlap due to direction
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulation - Dynamically adjust based on edge countNodeSpacing
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // 根据这对NodeDynamically adjust distance based on edge count between
      // Base距离 150, increase for each additional edge 40
    ...
    // Add center gravity to make isolatedNodecluster to center area
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loops
    if (d.isSelfLoop) {
      // Self-loop: draw an arc fromNode出发再返回
      const loopRadius = 30
      // 从NodeStart from right side, loop around and return
      const x1 = sx + 8  // 起点偏移
      const y1 = sy - 4
      const x2 = sx + 8  // 终点偏移
      const y2 = sy + 4
    ...
    // Perpendicular offset to link direction, calculated by distance ratio, ensuring curve visibility
    // More edges, greater offset-to-distance ratio
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // Base25%, increase for each additional edge5%
  ...
  // Calculate curve midpoint (forLabel定digits)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect if drag truly started (movement exceeds threshold)
  ...
  simulation.on('tick', () => {
    ...
    // 更新Edge Labelposition (no rotation, horizontal display is clearer)
