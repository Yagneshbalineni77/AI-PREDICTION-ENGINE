    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Create simulation instance, fetch simulation world parameter template
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Combine context to automatically call tools to extract entities and relations from the knowledge graph, initialize simulated individuals, and endow them with unique behaviors and memories based on reality seeds
          </p>
      ...
      <!-- Step 03: 生成双平台Simulation Config -->
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently sets world time flow, recommendation algorithms, active periods and posting frequencies for each individual, and event triggers based on simulation requirements and reality seeds.
          </p>
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              ...
              <div class="time-periods">
                ...
                <div class="period-item">
                  <span class="period-label">Off-peak</br>Hours</span>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Automatically generate initial activating events and hot topics based on narrative direction to guide the initial state of the simulated world
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Simulation Rounds Config - 只有在Configuration生成Completeonly show after rounds are calculated -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Rounds Setting</span>
                <span class="section-desc">MiroFish automatically plans simulation for <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> real-world hours, each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of time passing</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>If Agent scale is 100: Estimated time ~{{ Math.round(customMaxRounds * 0.6) }} min</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        If Agent scale is 100: Estimated time ~{{ Math.round(autoGeneratedRounds * 0.6) }} min
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">If running for the first time, it is highly recommended to switch to 'Custom Mode' to reduce rounds for a quick preview and lower error risk ➝</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            ...
            <div class="info-item">
              ...
              <span class="info-value">{{ { male: 'Male', female: 'Female', other: 'Other' }[selectedProfile.gender?.toLowerCase()] || selectedProfile.gender }}</span>
            </div>
            <div class="info-item">
...
// State
const phase = ref(0) // 0: Initialization, 1: Generate Personas, 2: Generate Config, 3: Complete
...
watch(currentStage, (newStage) => {
  if (newStage === 'Generate Agent Personas' || newStage === 'generating_profiles') {
    ...
    phase.value = 2
    // 进入Configuration生成Phase，开始轮询Configuration
    if (!configTimer) {
    ...
    phase.value = 2 // 仍属于ConfigurationPhase
  }
})

// 从ConfigurationCalculate auto-generated rounds (without hardcoded defaults)
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // Configuration未生成时返回 null
  }
  const totalHours = simulationConfig.value.time_config.total_simulation_hours
  const minutesPerRound = simulationConfig.value.time_config.minutes_per_round
  if (!totalHours || !minutesPerRound) {
    return null // Configuration数据不完整时返回 null
  }
...
const handleStartSimulation = () => {
  // BuildParameters passed to parent component
  const params = {}
  
  if (useCustomRounds.value) {
    // User custom rounds, pass max_rounds 参数
    params.maxRounds = customMaxRounds.value
    addLog(`Start simulation, custom rounds: ${customMaxRounds.value}`)
  } else {
    // 用户SelectionKeep auto-generated rounds, do not pass max_rounds 参数
    addLog(`Start simulation, using auto rounds: ${autoGeneratedRounds.value}`)
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Found completed preparation work, using directly')
      ...
      // Immediately set expected total agents (frompreparefrom API return value)
...
const pollPrepareStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 解析Phaseinfo and output detailed log
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 输出Configuration生成Phaselog (avoid duplicates)
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        lastLoggedConfigStage = data.generation_stage
        if (data.generation_stage === 'generating_profiles') {
          addLog('Generating agent persona configs...')
        } else if (data.generation_stage === 'generating_config') {
          addLog('Calling LLM to generate simulation parameters...')
        }
      }
...
const loadPreparedData = async () => {
  ...
  // Fetch once more Profiles
  await fetchProfilesRealtime()
  addLog(`Loaded ${profiles.value.length} Agent personas`)

  // 获取Configuration(using real-time API)
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // Configurationnot yet generated, start polling
        addLog('Generating config, polling to wait...')
        startConfigPolling()
        }
    }
  } catch (err) {
