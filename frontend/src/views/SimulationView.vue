...
const handleNextStep = (params = {}) => {
  ...
  // If custom rounds, pass through query params
  if (params.maxRounds) {
 ...
 * 当用户从 Step 3 Return to Step 2 default user wants to exit simulation
 */
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Environment not running, but process may still exist, check simulationStatus
    ...
    // 检查环境Statusfailure does not affect subsequent flow
...
onMounted(async () => {
  ...
  // Check and Close Running Simulation（用户从 Step 3 返回时）
