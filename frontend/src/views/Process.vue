    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          <!-- Graphvisualization (shown whenever data exists) -->
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  LLM analyzes document content and automatically generates ontology structure suitable for simulation (Entity types + Relation types)
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API Description</div>
                <div class="detail-content">
                  Based on generated ontology, automatically chunks document and builds Knowledge Graph via Zep API, extracting entities and relations.
                </div>
              </div>
...
const selectedItem = ref(null) // 选中的Node或边
...
// Initialization - 处理新建Project或Load Existing Project
const initProject = async () => {
  const paramProjectId = route.params.projectId
  
  if (paramProjectId === 'new') {
    // 新建Project：从 store 获取待上传的数据
...
// 处理新建Project - 调用 ontology/generate API
const handleNewProject = async () => {
  const pending = getPendingUpload()
  
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'No files pending upload, please return to homepage and restart'
    loading.value = false
    return
  }
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      // 启动GraphData polling (independent ofTaskStatus轮询）
      startGraphPolling()
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('✅ Graph built, loading full data...')
        
        stopPolling()
        stopGraphPolling()
        currentPhase.value = 2
        
        // Update Progress显示为CompleteStatus
        buildProgress.value = {
          progress: 100,
          message: 'Build Complete, Loading Graph Data...'
        }
        
        // 重新加载Project数据获取 graph_id
        const projectResponse = await getProject(currentProjectId.value)
...
const renderGraph = () => {
  ...
  // 创建NodeIDset used to filter valid edges
  ...
  // 绘制边（Containsclickable transparent wide line)
