...
// 直接在Initialization时从 query 参数获取 maxRounds, ensure child components get value immediately
...
const handleNextStep = () => {
  // Step3Simulation 组件会直接处理Report Gen和路由跳转
  // 这个Method仅作为备用
...
const loadSimulationData = async () => {
  try {
    ...
    if (simRes.success && simRes.data) {
      ...
      // Get simulation config 以获取 minutes_per_round
      try {
        ...
        addLog(`Failed to fetch time config, using default: ${minutesPerRound.value} mins/round`)
      }
...
const loadGraph = async (graphId) => {
  // When simulating, auto-refresh hides full-screen loading to avoid flickering
  // Shown during manual refresh or initial load loading
  if (!isSimulating.value) {
...
onMounted(() => {
  ...
  // 记录 maxRounds Configuration（值已在Initialization时从 query 参数获取）
  if (maxRounds.value) {
