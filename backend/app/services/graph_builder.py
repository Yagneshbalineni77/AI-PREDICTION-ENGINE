...
from .graph_store import GraphStore, EpisodeData, EntityEdgeSourceTarget
...
class GraphBuilderService:
    """
    Graph Building Service
    负责调用Zep APIBuild知识Graph
    """
    
    def __init__(self, api_key: Optional[str] = None):
        self.api_key = api_key or Config.ZEP_API_KEY or "local-sqlite-mode"
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        ...
        # Simple base classes replacing zep_cloud ontology models
        # GraphStore.set_ontology() only needs the class name and __doc__
        ...
        # Zep reserved names, 不能作为Attribute名
        RESERVED_NAMES = {'uuid', 'name', 'group_id', 'name_embedding', 'summary', 'created_at'}
        
        def safe_attr_name(attr_name: str) -> str:
            """将保留NameConvert为安全Name"""
        ...
        for entity_def in ontology.get("entity_types", []):
            ...
            # Create attribute dict and type annotations(Pydantic v2 需要)
            ...
            for attr_def in entity_def.get("attributes", []):
                ...
                attrs[attr_name] = None  # Simple default
        ...
        for edge_def in ontology.get("edge_types", []):
            ...
            for attr_def in edge_def.get("attributes", []):
                ...
                attr_desc = attr_def.get("description", attr_name)
                attrs[attr_name] = None  # Simple default
                annotations[attr_name] = Optional[str]  # 边Attribute用strType
    ...
    def _wait_for_episodes(
        ...
        if not episode_uuids:
            if progress_callback:
                progress_callback("无需Wait(没有 episode)", 1.0)
            return
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        """
        Get完整GraphData(包含详细Info)
        
        Args:
            graph_id: Graph ID
            
        Returns:
            包含nodes和edges的Dict, 包括时间Info, Attribute等详细Data
        """
        nodes = fetch_all_nodes(self.client, graph_id)
        edges = fetch_all_edges(self.client, graph_id)
