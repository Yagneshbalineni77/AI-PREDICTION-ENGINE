...
**IMPORTANT: You must output valid JSON data only. Do not output anything else. ALL text must be in English.**
...
- We need to simulate how different parties react and how information spreads during opinion events
    ...
    "entity_types": [
        {
            ...
            "description": "Brief description (English, max 20 characters)"
        }
    ],
    "edge_types": [
        {
...
**Quantity requirement: Must be exactly 5 entity types**

**Hierarchy requirements (must include both specific types and fallback types)**:

Your 5 entity types must include the following layers:

A. **Fallback types (must include, place as last 2 in the list)**:
   - `Person`: Fallback type for any natural person. When a person doesn't belong to any more specific person type, classify them here.
   - `Organization`: Fallback type for any organization. When an organization doesn't belong to any more specific organization type, classify them here.

B. **Specific types (3, designed based on text content)**:
...
- Various people appear in text, such as "elementary teachers", "bystanders", "a netizen"
...
**Individual (Fallback)**:
- Person: Any natural person (used when not matching above specific types)
...
- Organization: Any organization (used when not matching above specific types)
...
class OntologyGenerator:
    ...
    # Max text length for LLM (50k chars)
...
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for edge in result["edge_types"]:
            if "source_targets" not in edge:
                edge["source_targets"] = []
            if len(edge.get("source_targets", [])) > 10:
    ...
    def generate_python_code(self, ontology: Dict[str, Any]) -> str:
        ...
        code_lines = [
            ...
            '"""',
            '',
            '# Base classes for ontology (no external dependencies needed)',
            'class EntityModel: pass',
            'class EntityText: pass',
